...
3) In a class using the ABAP language version *ABAP for Cloud Development*, you can insert the following code snippet.
   ...
   - This snippet is intended to illustrate the use of a released APIs while being cautious about typing. An API may be extended in the future, which can affect its usage. In the example below, a manually created type is used that could compromise type compatibility in future.
