...
**********************************************************************
  ...
  METHOD m21_builtin_dobj_b.
    ...
    DATA syfdpos LIKE sy-fdpos.
    IF test_string CS `p`.
      syfdpos = sy-fdpos.
      out->write( |The substring is found. Offset of first finding: { syfdpos }| ).
    ELSE.
      syfdpos = sy-fdpos.
      out->write( |The substring is not found. Length of searched string: { syfdpos }| ).
      ASSERT syfdpos = strlen( test_string ).
    ENDIF.
