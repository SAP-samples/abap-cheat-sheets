"! <p class="shorttext"><strong>Numeric Operations in ABAP</strong><br/>ABAP cheat sheet example class</p>
"!
"! <p>The example class demonstrates numeric operations in ABAP.<br/>
...
"! <li>Disclaimer</li></ul>
CLASS zcl_demo_abap_numeric_op DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_calculation IMPORTING num        TYPE numeric
                                     RETURNING VALUE(ref) TYPE REF TO zcl_demo_abap_numeric_op.
...
CLASS zcl_demo_abap_numeric_op IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

 zcl_demo_abap_aux=>set_example_divider(
      out  = out
      text = `ABAP Cheat Sheet Example: Numeric Operations`
    ).

out->write( `--------------------------- Note ---------------------------` ).
out->write( `The example is set up to display output in the console only ` ).
out->write( `for the calculator snippet. To explore the effect of statements ` ).
out->write( `and content of variables, you may want to set break points ` ).
out->write( `and walk through the code in the debugger.` ).
out->write( `------------------------------------------------------------` ).
out->write( |\n| ).
...
*&---------------------------------------------------------------------*
  ...
  METHOD create_calculation.
    ref = NEW zcl_demo_abap_numeric_op( ).
    ref->number = num.
  ENDMETHOD.
ENDCLASS.
