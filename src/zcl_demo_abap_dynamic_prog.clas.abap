...
**********************************************************************
    ...
    "Upcast
    ref_data_d1 = ref_d3.
    ASSIGN ref_data_d1->* TO FIELD-SYMBOL(<a>).
...
**********************************************************************
    ...
    ASSIGN ref_h1->* TO FIELD-SYMBOL(<b>).
    output->display( input = <b> name = `<b>` ).
...
**********************************************************************
    ...
    ASSIGN COMPONENT comp_name OF STRUCTURE wa TO FIELD-SYMBOL(<fs_m6>).

    ASSIGN COMPONENT 'CARRNAME' OF STRUCTURE wa TO FIELD-SYMBOL(<fs_m7>).
    ...
    ASSIGN COMPONENT 'CRRNM' OF STRUCTURE wa TO FIELD-SYMBOL(<fs_m8>).
    ...
    ASSIGN COMPONENT 4 OF STRUCTURE wa TO FIELD-SYMBOL(<fs_m9>).

    "If the value is 0, the memory area of the entire structure is assigned to the field symbol.
    ASSIGN COMPONENT 0 OF STRUCTURE wa TO FIELD-SYMBOL(<fs_m10>).
...
**********************************************************************
    ...
    TRY.
        ...
        ASSIGN ref_bt->* TO FIELD-SYMBOL(<f>).
...
**********************************************************************
    ...
    ASSIGN ref_dynstruc->* TO FIELD-SYMBOL(<g>).
...
**********************************************************************
    ...
    AND dyn_syntax_elem-rows IS NOT INITIAL.
      ...
      FIELD-SYMBOLS <i> TYPE ANY TABLE.
      ASSIGN dyn_syntax_elem-target->* TO <i>.
...
**********************************************************************
    ...
    AND get_type <> `IF_OO_ADT_CLASSRUN`.
      ...
      ASSIGN dref->* TO FIELD-SYMBOL(<j>).
      DATA(some_type) = cl_abap_typedescr=>describe_by_data( <j> ).
...
**********************************************************************
    ...
    ASSIGN dref_struc->* TO FIELD-SYMBOL(<k>).
    ASSIGN COMPONENT column1 OF STRUCTURE <k> TO FIELD-SYMBOL(<l>).
    ASSIGN COMPONENT column2 OF STRUCTURE <k> TO FIELD-SYMBOL(<m>).
    ASSIGN COMPONENT column3 OF STRUCTURE <k> TO FIELD-SYMBOL(<n>).
    <l> = 'abc'.
    <m> = 'def'.
    <n> = 'ghi'.

    output->display( input = <k> name = `<k>` ).
...
**********************************************************************
    ...
    FIELD-SYMBOLS <o> TYPE ANY TABLE.
    ASSIGN ref_tab->* TO <o>.
    ...
    SELECT *
      ...
      INTO CORRESPONDING FIELDS OF TABLE @<o>
      UP TO 3 ROWS.
